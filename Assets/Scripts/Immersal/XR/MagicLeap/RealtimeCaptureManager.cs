...
namespace Immersal.XR.MagicLeap
{
    public class RealtimeCaptureManager : MonoBehaviour
    {
        ...
        /*async */void Start()
        {
            ...
            Immersal.Core.SetInteger("MagicUserLevel", 1);
...
/*            if (m_CameraDataProvider == null)
            {
                Debug.LogError("No camera data provider, disabling...");
                enabled = false;
            }

            if (!m_CameraDataProvider.IsCameraConnected)
            {
                m_StatusText.text = "Waiting for camera...";

                Task t = Task.Run(async () =>
                {
                    while (!m_CameraDataProvider.IsCameraConnected)
                    {
                ...
                m_StatusText.text = "Camera connected";
        ...
        private async Task RealtimeCapture()
        {
            if (m_CameraDataProvider.TryAcquireLatestData(out CaptureData data))
            {
                ...
                Task<int> t = Task.Run(() =>
                {
                    return Immersal.Core.MapAddImage(data.PixelBuffer, data.Width, data.Height, channels, ref data.Intrinsics, ref camPos, rot);
                });
